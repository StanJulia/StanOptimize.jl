struct CmdStanOptimizeModel
  ...
  settings::StanOpti.SamplerSettings
end

function CmdStanOptimizeModel(name::AbstractString, model::AbstractString;
  tmpdir = mktempdir(), 
  settings = StanOptimize.optimize_settings)
  ...
  stan_compile(sm)
  CmdStanOptimizeModel(name, model, tmpdir, output_base, sm, settings)
end

function Base.show(io::IO, model::CmdStanOptimizeModel)
